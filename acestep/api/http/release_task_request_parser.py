...
async def parse_release_task_request(
    ...
    if content_type.startswith("multipart/form-data"):
        try:
            ...
            print(
                f"[API Server] Multipart form received: "
                f"ref_upload={type(ref_upload).__name__ if ref_upload else None}, "
                f"ctx_upload={type(ctx_upload).__name__ if ctx_upload else None}"
            )
            if isinstance(ref_upload, upload_file_type):
                ...
                print(f"[API Server] Reference audio saved to temp: {reference_audio_path}")
            else:
